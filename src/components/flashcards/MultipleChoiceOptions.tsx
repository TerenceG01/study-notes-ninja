    ...
    <div className="space-y-2 sm:space-y-3 w-full max-w-full">
      {options.slice(0, 5).map((option) => (
        <Button
          ...
          className={`w-full justify-start text-left h-auto ${isMobile ? 'py-2 px-3 text-xs' : 'py-3 px-4 sm:px-5 text-sm'} whitespace-normal`}
      ...
      {isAnswered && options.find(o => o.id === selectedOption)?.explanation && (
        <div className="mt-3 p-2 sm:p-3 bg-muted rounded-lg">
          <p className="text-xs break-words line-clamp-3">{options.find(o => o.id === selectedOption)?.explanation}</p>
        </div>
      )}
    </div>
  );
